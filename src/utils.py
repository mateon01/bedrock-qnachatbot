...
application_metadata = {
    ...
    'models-emb':[
        {'name':'Titan', 'endpoint':'amazon.titan-embed-text-v2:0'},
        ],
...
def select_models(page):
     with st.sidebar:
        ...
        max_len = st.slider('Max Length', min_value=50, max_value=8192, value=300, step=10)
        top_p = st.slider('Top p', min_value=0., max_value=1., value=1., step=.01)
        top_k = st.slider('Top k', min_value=0, max_value=500, value=250, step=10)
        temp = st.slider('Temperature', min_value=0., max_value=1., value=0.01, step=.01)
        
        if page == "rag" and "OpenSearch" in retriever:
            params = {'action_name': 'Document Quer', 'endpoint-llm': MODELS_LLM[llm_model_name], 'max_len': max_len, 'top_k': top_k, 'top_p': top_p, 'temp': temp,
                      'model_name':llm_model_name, "rag":retriever,"K":K, "engine":engine, "m":m,
            ...
            params = {'model_name': llm_model_name, 'endpoint-llm':MODELS_LLM[llm_model_name],'max_len':max_len, 'top_p':top_p, 'top_k':top_k, 'temp':temp,'action_name': "Document Query",'Bucket': BUCKET,'Prefix': PREFIX,"rag":retriever,'Region_Name': REGION}
        else:
            params = {'model_name': llm_model_name, 'endpoint-llm':MODELS_LLM[llm_model_name],'max_len':max_len, 'top_p':top_p, 'top_k':top_k, 'temp':temp,'action_name': "Document Query",'Bucket': BUCKET,'Prefix': PREFIX,"rag":"",'Region_Name': REGION}
